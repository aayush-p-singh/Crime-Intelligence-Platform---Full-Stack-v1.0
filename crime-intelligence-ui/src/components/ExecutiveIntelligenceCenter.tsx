    ...
    <div className="mt-5 space-y-5 border-t border-white/10 pt-5">
      <div>
        ...
        <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
          ...
          ].map(([label, value]) => <div key={String(label)} className="rounded-lg border border-white/10 bg-slate-950/40 p-3"><p className="text-[10px] uppercase tracking-wider text-slate-500">{label}</p><p className="mt-1 truncate text-sm font-bold text-slate-200">{value}</p></div>)}
        </div>
        <p className="mt-3 text-xs leading-5 text-slate-400">{assessment}</p>
      </div>

      <div className="grid gap-5 lg:grid-cols-2">
        <div><h3 className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-500">Threat Distribution</h3><div className="space-y-2">{distribution.map((item) => <div key={item.category}><div className="mb-1 flex justify-between text-xs text-slate-400"><span>{item.category}</span><span>{item.count} articles</span></div><div className="h-1.5 overflow-hidden rounded-full bg-white/10"><div className="h-full bg-cyan-400" style={{ width: `${item.percentage}%` }} /></div></div>)}</div></div>
