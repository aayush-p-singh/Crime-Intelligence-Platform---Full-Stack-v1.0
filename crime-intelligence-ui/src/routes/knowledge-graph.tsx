            ...
            {!selectedNode ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center opacity-50">
                ...
                <p className="text-sm text-slate-400">
                  Click any node in the graph to inspect its metadata and relationships.
                </p>
              </div>
            ) : (
              <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                <div>
                  <span
                    ...
                    style={{
                      color: NODE_COLORS[selectedNode.group || "Unknown"] || NODE_COLORS["Unknown"],
                    }}
                  >
                ...
                <div className="space-y-3">
                  <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-white/5 pb-2">
                    Properties
                  </h4>
                  ...
                  {Object.entries(selectedNode).map(([key, value]) => {
                    if (["id", "label", "group", "x", "y", "vx", "vy", "index"].includes(key))
                      return null;
                    return (
                      <div
                        key={key}
                        className="bg-white/5 border border-white/10 rounded-lg p-3 break-words"
                      >
                        <span className="text-xs text-slate-400 block mb-1 capitalize">
                          {key.replace("_", " ")}
                        </span>
                  ...
                  <div className="pt-4 border-t border-white/10">
                    ...
                    <div className="flex items-center justify-between bg-blue-500/10 border border-blue-500/20 rounded-lg p-3">
                      ...
                      <span className="text-lg font-bold text-blue-400">
                        {neighbors.get(selectedNode.id)?.nodes.size}
                      </span>
                    </div>
                  </div>
                )}
