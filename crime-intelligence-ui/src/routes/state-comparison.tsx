...
function StateComparisonComponent() {
  ...
  const barChartData = hasValidData
    ? [
        ...
        {
          name: "Chargesheet %",
          [state1]: compData[0].chargesheetRate,
          [state2]: compData[1].chargesheetRate,
        },
      ]
    : [];
  ...
  const radarData = hasValidData
    ? [
        {
          metric: "Risk Score (Norm)",
          [state1]: compData[0].crimeRate / 10,
          [state2]: compData[1].crimeRate / 10,
        },
        {
          metric: "Women Crime (Norm)",
          [state1]: compData[0].womenCrime / 1000,
          [state2]: compData[1].womenCrime / 1000,
        },
        {
          metric: "Chargesheet Rate",
          [state1]: compData[0].chargesheetRate,
          [state2]: compData[1].chargesheetRate,
        },
      ]
        ...
        <div className="bg-white/80 backdrop-blur-sm border border-black/[0.06] rounded-xl p-6 shadow-lg flex flex-col md:flex-row gap-6 items-center">
          <div className="flex-1 w-full">
            ...
            >
              ...
              {stateList?.map((s) => (
                <option key={s} value={s} disabled={s === state2}>
                  {s}
                </option>
              ))}
            </select>
          ...
          <div className="flex-1 w-full">
            ...
            >
              <option value="">Select State...</option>
              {stateList?.map((s) => (
                <option key={s} value={s} disabled={s === state1}>
                  {s}
                </option>
              ))}
            </select>
          ...
          <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <ComparisonCard
                ...
                value1={compData[0].totalCrime}
                value2={compData[1].totalCrime}
              />
              <ComparisonCard
                label="Crime Rate"
                value1={compData[0].crimeRate}
                value2={compData[1].crimeRate}
              />
              <ComparisonCard
                label="Crimes Against Women"
                value1={compData[0].womenCrime}
                value2={compData[1].womenCrime}
              />
              <ComparisonCard
                label="Chargesheet Rate"
                value1={compData[0].chargesheetRate}
                value2={compData[1].chargesheetRate}
                suffix="%"
              />
            </div>
