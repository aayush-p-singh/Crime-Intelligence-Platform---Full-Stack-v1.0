...
function AnimatedSeverityBar({ score, tone }: { score: number; tone: string }) {
  ...
  return (
    <div ref={ref} className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/10">
      <div
...
function ExecutiveBriefingCard({
    ...
    return (
      ...
      >
        <div className="mb-5 flex items-center gap-3">
          <div className="briefing-skeleton h-10 w-10 rounded-lg" />
          <div className="space-y-2">
            <div className="briefing-skeleton h-4 w-64 rounded" />
            <div className="briefing-skeleton h-3 w-44 rounded" />
          </div>
        </div>
        <div className="grid gap-3 md:grid-cols-4">
          {Array.from({ length: 4 }, (_, index) => (
            <div key={index} className="briefing-skeleton h-16 rounded-lg" />
          ))}
        </div>
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          {Array.from({ length: 4 }, (_, index) => (
            <div key={index} className="briefing-skeleton h-20 rounded-lg" />
          ))}
        </div>
      </div>
    );
  ...
  const refreshProgress = refreshStartedAt
    ? Math.min(Math.floor((now - refreshStartedAt) / 650), refreshSteps.length - 1)
    : 0;
  ...
  return (
    <section className="intelligence-card intelligence-card-hover rounded-xl border border-black/[0.06] bg-white/60 p-6 shadow-lg backdrop-blur-md page-enter">
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3 text-xs">
        <span
          className={`rounded-full border px-3 py-1 font-bold uppercase tracking-wider ${riskClasses(briefing.riskLevel)}`}
        >
...
function DashboardComponent() {
  ...
  const avgCrimeRate =
    activeData.length > 0
      ? (activeData.reduce((sum, state) => sum + state.crimeRate, 0) / activeData.length).toFixed(1)
      : "0.0";
  const avgChargesheet =
    activeData.length > 0
      ? (
          activeData.reduce((sum, state) => sum + state.chargesheetRate, 0) / activeData.length
        ).toFixed(1)
      : "0.0";

  const top5CrimeStates = [...activeData].sort((a, b) => b.crimeRate - a.crimeRate).slice(0, 5);
  const bottom5Chargesheet = [...activeData]
    .sort((a, b) => a.chargesheetRate - b.chargesheetRate)
    .slice(0, 5);
  ...
  const { data: forecastData, isLoading: isForecasting } = useQuery({
    ...
    queryFn: async () => {
      ...
      const inputCrimeRate =
        selectedRegion === "All" ? parseFloat(avgCrimeRate) : activeData[0]?.crimeRate || 0;
      const inputWomenCrime =
        selectedRegion === "All"
          ? Math.round(totalWomenCrimes / activeData.length)
          : activeData[0]?.womenCrime || 0;
      const inputChargesheet =
        selectedRegion === "All" ? parseFloat(avgChargesheet) : activeData[0]?.chargesheetRate || 0;
    ...
    try {
      ...
      const dataUrl = await toPng(reportRef.current, {
        ...
        filter: (node) => {
          ...
          if (el?.tagName === "LINK" && el?.getAttribute("href")?.includes("fonts.googleapis"))
            return false;
          return true;
        },
      });
      ...
      pdf.text(
        `Scope: ${selectedRegion === "All" ? "National overview" : selectedRegion}`,
        margins,
        115,
      );
      ...
      if (briefing) {
        ...
        metric(
          "Evidence quality",
          briefing.confidenceEvidence?.evidenceQuality || "Unavailable",
          margins + 94,
          y,
          42,
        );
        ...
        writeBlock(
          "Executive briefing data was unavailable at export time.",
          margins,
          y,
          contentWidth,
        );
      ...
      if (briefing) {
        ...
        briefing.severityMatrix?.forEach((item, index) => {
          ...
          pdf.setFillColor(
            item.score >= 70 ? 220 : item.score >= 40 ? 245 : 16,
            item.score >= 70 ? 38 : item.score >= 40 ? 158 : 185,
            item.score >= 70 ? 38 : item.score >= 40 ? 11 : 129,
          );
          pdf.roundedRect(
            margins + 45,
            rowY - 4,
            (contentWidth - 65) * Math.min(item.score / 100, 1),
            4,
            1,
            1,
            "F",
        ...
        for (const day of briefing.threatTimeline || []) {
          if (y > pageHeight - 35) {
            pdf.addPage();
            y = heading("Threat Timeline (continued)", 22);
          }
      ...
      if (briefing) {
        ...
        briefing.sources.forEach((source) =>
          publisherCounts.set(
            source.sourceName || "Unknown publisher",
            (publisherCounts.get(source.sourceName || "Unknown publisher") || 0) + 1,
          ),
        );
        const topPublisher = [...publisherCounts.entries()].sort((a, b) => b[1] - a[1])[0];
        y = writeBlock(
          `Most frequent publisher: ${topPublisher?.[0] || "Unavailable"} (${topPublisher?.[1] || 0} articles)\nEvidence quality: ${briefing.confidenceEvidence?.evidenceQuality || "Unavailable"}\nRecency: ${briefing.confidenceEvidence?.recency || "Unavailable"}\nConfidence assessment: ${briefing.confidenceEvidence?.assessment || "Unavailable"}`,
          margins,
          y,
          contentWidth,
        );
        ...
        for (const source of briefing.sources) {
          if (y > pageHeight - 30) {
            pdf.addPage();
            y = heading("Sources Consulted (continued)", 22);
          }
          y = writeBlock(
      ...
      for (let page = 1; page <= totalPages; page += 1) {
        ...
        pdf.text(`Page ${page} of ${totalPages}`, pageWidth - margins, pageHeight - 10, {
          align: "right",
        });
      }
            ...
            <>
              ...
              <div className="bg-gradient-to-r from-indigo-950 to-slate-900 border border-indigo-500/30 p-6 rounded-xl shadow-2xl relative overflow-hidden z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                ...
                <div className="relative z-10 flex flex-col items-end">
                  ...
                  {isForecasting ? (
                    ...
                    <span
                      className={`text-4xl font-black uppercase tracking-widest ${threatColor}`}
                    >
                      {threatLevel}
                    </span>
                  )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 relative z-10">
                ...
                <KPICard
                  ...
                  value={totalWomenCrimes.toLocaleString()}
                  icon={Users}
                />
                <KPICard
                  title={
                    selectedRegion === "All" ? "Highest Threat Region" : "Current Threat Level"
                  }
                  value={
                    selectedRegion === "All"
                      ? top5CrimeStates[0]?.name || "N/A"
                      : activeData[0]?.risk || "N/A"
                  }
                  icon={ShieldAlert}
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={bottom5Chargesheet}>
                        ...
                        <Line
                          ...
                          stroke="#ef4444"
                          strokeWidth={3}
                          dot={{ r: 6, fill: "#0f172a", stroke: "#ef4444", strokeWidth: 2 }}
                        />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
