...
function LoginComponent() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err.message || "AUTH_FAILURE: Security handshake dropped by security perimeter framework.",
      );
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="w-full max-w-md animate-in fade-in zoom-in-95 duration-500 relative z-10">
        ...
        <div className="bg-[#15171b] border border-[#2a2e36] rounded p-8 shadow-lg relative">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <div>
              <label className="block text-xs font-bold text-[#b6bdc9] uppercase tracking-widest mb-2">
                Operator ID
              </label>
        ...
        <div className="mt-8 text-center px-4 space-y-2">
          <div className="text-[10px] text-[#7e8795] font-mono tracking-wide bg-[#111317] inline-block px-3 py-1 rounded border border-[#2a2e36]">
            DEMO ACCESS: <span className="text-[#8daed1]">admin</span> /{" "}
            <span className="text-[#8daed1]">admin123</span>
          </div>
          <div className="text-[10px] text-[#7e8795] leading-relaxed font-mono tracking-wide max-w-sm mx-auto">
            WARNING: Authorized Personnel Only. All diagnostic data intercepts, transaction inputs,
            and IP session footprints are logged globally.
          </div>
        </div>
      </div>
    </div>
